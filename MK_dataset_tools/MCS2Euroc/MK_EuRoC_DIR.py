...
def copy_images(src_dir, dst_dir):
    ...
    print(f"已从 {src_dir} 复制 {copied_count} 个图片文件到 {dst_dir}")
    return copied_count
...
def mk_euroc_dir(DATA_DIR,SRC_DIR_CAM0,SRC_DIR_CAM1):
    ...
    if not os.path.exists(src_dir_0):
        ...
        # 复制图片
        copied_cam0 = copy_images(src_dir_0, dst_dir_cam0)
    ...
    if not os.path.exists(src_dir_1):
        ...
        # 复制图片
        copied_cam1 = copy_images(src_dir_1, dst_dir_cam1)
    
    print(f"总共复制了 {copied_cam0 + copied_cam1} 个图片文件")
    if copied_cam0 > 0:
        print(f"- 从 {src_dir_0} 到 {dst_dir_cam0}: {copied_cam0} 个文件")
    if copied_cam1 > 0:
        print(f"- 从 {src_dir_1} 到 {dst_dir_cam1}: {copied_cam1} 个文件")
